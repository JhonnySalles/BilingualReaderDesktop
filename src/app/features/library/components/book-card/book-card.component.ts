...
@Component({
  ...
  template: `
    ...
    @if (cardStyle === 'STANDARD') {
      <div class="group relative bg-slate-800/60 backdrop-blur-md rounded-xl overflow-hidden border border-slate-700/50 hover:border-amber-500/50 transition-all duration-300 hover:shadow-xl hover:shadow-amber-500/10 hover:-translate-y-1 cursor-pointer flex flex-col h-full">
        ...
        <div class="relative aspect-[2/3] w-full overflow-hidden bg-slate-900 cover-3d-host">
          @if (book.coverPath) {
            <img [src]="'local-cover:///' + book.coverPath" [alt]="book.title" class="cover-3d-face w-full h-full object-cover group-hover:scale-105 transition-transform duration-500" />
          } @else {
          ...
          <div class="absolute top-2 left-2 right-2 flex justify-between items-center z-10 pointer-events-auto">
            <span class="px-2 py-0.5 text-[10px] font-bold rounded-md bg-slate-950/80 backdrop-blur-md text-amber-300 border border-amber-500/30 uppercase tracking-wider shadow">
              {{ book.fileType || 'EPUB' }}
            </span>
            ...
            <div class="flex items-center gap-1.5">
              <button
                ...
                [class.opacity-100]="book.favorite"
                [class.opacity-0]="!book.favorite"
                ...
                <svg xmlns="http://www.w3.org/2000/svg" class="w-3.5 h-3.5" [class.fill-current]="book.favorite" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
        ...
        <div class="p-3 flex flex-col flex-1 justify-between">
          <div>
            <h3 class="text-sm font-semibold text-slate-100 line-clamp-2 sm:line-clamp-3 md:line-clamp-4 lg:line-clamp-5 group-hover:text-amber-400 transition-colors" [title]="book.title">
              {{ book.title }}
            </h3>
    ...
    @if (cardStyle === 'OVERLAY') {
      <div class="group relative aspect-[2/3] w-full rounded-xl overflow-hidden border border-slate-700/50 hover:border-amber-500/50 transition-all duration-300 hover:shadow-xl hover:shadow-amber-500/10 hover:-translate-y-1 cursor-pointer flex flex-col justify-between bg-slate-900">
        @if (book.coverPath) {
          ...
          <div class="absolute inset-0 w-full h-full flex flex-col items-center justify-center p-4 text-amber-500/70 bg-gradient-to-br from-slate-900 via-slate-900 to-amber-950/30">
            <svg xmlns="http://www.w3.org/2000/svg" class="w-12 h-12 mb-2 opacity-75" fill="none" viewBox="0 0 24 24" stroke="currentColor">
              <path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M12 6.253v13m0-13C10.832 5.477 9.246 5 7.5 5S4.168 5.477 3 6.253v13C4.168 18.477 5.754 18 7.5 18s3.332.477 4.5 1.253m0-13C13.168 5.477 14.754 5 16.5 5c1.747 0 3.332.477 4.5 1.253v13C19.832 18.477 18.247 18 16.5 18c-1.746 0-3.332.477-4.5 1.253" />
            </svg>
            <span class="text-xs text-center font-medium opacity-75 uppercase tracking-wider">{{ book.fileType || 'EPUB' }}</span>
        ...
        <div class="bg-slate-900 border border-slate-700 rounded-2xl p-5 max-w-md w-full shadow-2xl flex flex-col gap-4 text-center">
          ...
          <div>
            ...
            <p class="text-xs text-slate-400 mt-1 break-words">
              Deseja remover <strong>"{{ book.title }}"</strong> da biblioteca?
            </p>
          </div>
...
export class BookCardComponent implements OnDestroy {
  ...
  getProgressPercentage(): number {
    return progressPercent(this.book.bookMark || 0, this.book.pages || 0, this.book.completed);
  }
  ...
  onSetBookmark(event: MouseEvent): void {
    event.stopPropagation();
    this.isMenuOpen.set(false);
    this.setBookmark.emit(this.book);
  }

  onClearProgress(event: MouseEvent): void {
    event.stopPropagation();
    this.isMenuOpen.set(false);
    this.bookService.clearProgress(this.book);
  }
