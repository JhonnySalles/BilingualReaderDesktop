...
class MangaRepository extends base_repository_1.BaseRepository {
    ...
    getByCoverPath(coverPath) {
        ...
        const stmt = this.db.prepare(`SELECT * FROM Manga WHERE cover_path = ? OR cover_path = ? OR cover_path = ? OR LOWER(cover_path) = LOWER(?) OR cover_path LIKE ? LIMIT 1`);
    ...
    save(manga) {
        ...
        if (manga.id) {
            ...
            const resolvedType = merged.fileType || (merged.path ? path.extname(merged.path).toUpperCase().replace('.', '') : app_enums_1.FileType.CBZ);
        ...
        WHERE id = ?
      `);
            stmt.run(resolvedTitle, merged.path, resolvedFolder, resolvedName, merged.fileSize ?? 0, resolvedType, merged.pages ?? 1, chaptersJson, chaptersPagesJson, merged.bookMark ?? 0, merged.completed ? 1 : 0, merged.favorite ? 1 : 0, merged.hasSubtitle ? 1 : 0, merged.author ?? '', merged.series ?? '', merged.genre ?? '', merged.publisher ?? '', merged.volume ?? '', merged.release ?? null, merged.language ?? '', merged.storyArch ?? '', merged.characters ?? '', merged.tags ?? '', merged.fkLibrary ?? null, merged.excluded ? 1 : 0, merged.lastAccess ?? null, merged.lastAlteration ?? new Date().toISOString(), resolvedFileAlteration, merged.lastVocabImport ?? null, merged.lastVerify ?? null, merged.coverPath ?? null, manga.id);
            return manga.id;
        }
        else {
            ...
            const resolvedType = manga.fileType || (manga.path ? path.extname(manga.path).toUpperCase().replace('.', '') : app_enums_1.FileType.CBZ);
      ...
      `);
            const info = stmt.run(resolvedTitle, manga.path, resolvedFolder, resolvedName, manga.fileSize ?? 0, resolvedType, manga.pages ?? 1, chaptersJson, chaptersPagesJson, manga.bookMark ?? 0, manga.completed ? 1 : 0, manga.favorite ? 1 : 0, manga.hasSubtitle ? 1 : 0, manga.author ?? '', manga.series ?? '', manga.genre ?? '', manga.publisher ?? '', manga.volume ?? '', manga.release ?? null, manga.language ?? '', manga.storyArch ?? '', manga.characters ?? '', manga.tags ?? '', manga.fkLibrary ?? null, manga.excluded ? 1 : 0, new Date().toISOString(), manga.lastAccess ?? null, manga.lastAlteration ?? new Date().toISOString(), resolvedFileAlteration, manga.lastVocabImport ?? null, manga.lastVerify ?? null, manga.coverPath ?? null);
