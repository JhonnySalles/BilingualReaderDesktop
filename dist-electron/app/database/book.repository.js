...
class BookRepository extends base_repository_1.BaseRepository {
    ...
    save(book) {
        ...
        WHERE id = ?
      `);
            stmt.run(book.title, book.path, book.folder, book.name, book.fileSize ?? 0, book.fileType, book.pages ?? 1, book.bookMark ?? 0, book.completed ? 1 : 0, book.favorite ? 1 : 0, book.author ?? '', book.series ?? '', book.genre ?? '', book.publisher ?? '', book.volume ?? '', book.release ?? null, book.fkLibrary ?? null, book.excluded ? 1 : 0, book.lastAccess ?? null, book.lastAlteration ?? new Date().toISOString(), book.fileAlteration ?? new Date().toISOString(), book.lastVocabImport ?? null, book.lastVerify ?? null, book.id);
            return book.id;
        }
        else {
            const stmt = this.db.prepare(`
      ...
      `);
            const info = stmt.run(book.title, book.path, book.folder, book.name, book.fileSize ?? 0, book.fileType, book.pages ?? 1, book.bookMark ?? 0, book.completed ? 1 : 0, book.favorite ? 1 : 0, book.author ?? '', book.series ?? '', book.genre ?? '', book.publisher ?? '', book.volume ?? '', book.release ?? null, book.fkLibrary ?? null, book.excluded ? 1 : 0, new Date().toISOString(), book.lastAccess ?? null, book.lastAlteration ?? new Date().toISOString(), book.fileAlteration ?? new Date().toISOString(), book.lastVocabImport ?? null, book.lastVerify ?? null);
