...
import {
  LinkedFile,
  LinkedPage,
  createEmptyLinkedPage
} from '../../src/app/core/models/entities/linked-file.model';
import { Languages } from '../../src/app/core/models/enums/app-enums';
import { PAGE_EMPTY } from '../../src/app/core/models/enums/page-link-enums';

export class FileLinkRepository {
  constructor(private db: Database.Database) {}

  private mapFileLink(row: any): LinkedFile {
    return {
      id: row.id,
      ...
      type: row.type || '',
      folder: row.folder || '',
      language: (row.language as Languages) || Languages.PORTUGUESE,
  ...
  private mapPageLink(row: any): LinkedPage {
    return createEmptyLinkedPage({
      ...
      fileLinkRightPage: row.file_right_link_page ?? PAGE_EMPTY,
  ...
  private getPagesLink(idFile: number): LinkedPage[] {
    const stmt = this.db.prepare(
      `SELECT * FROM PagesLink WHERE id_file = ? AND not_linked = 0 ORDER BY manga_page ASC, id ASC`
    );
    return (stmt.all(idFile) as any[]).map(row => this.mapPageLink(row));
  }

  private getPagesNotLink(idFile: number): LinkedPage[] {
    const stmt = this.db.prepare(
      `SELECT * FROM PagesLink WHERE id_file = ? AND not_linked = 1 ORDER BY file_link_page ASC, id ASC`
    );
    return (stmt.all(idFile) as any[]).map(row => this.mapPageLink(row));
  }

  private insertPages(idFile: number, pages: LinkedPage[]): void {
    const stmt = this.db.prepare(`
    ...
    for (const page of pages) {
      const result = stmt.run(
        idFile,
        page.mangaPage ?? PAGE_EMPTY,
        page.mangaPages ?? 0,
        page.mangaPageName || '',
        page.mangaPagePath || '',
        page.fileLinkLeftPage ?? PAGE_EMPTY,
        page.fileLinkLeftPages ?? 0,
        page.fileLinkLeftPageName || '',
        page.fileLinkLeftPagePath || '',
        page.fileLinkRightPage ?? PAGE_EMPTY,
        page.fileLinkRightPageName || '',
        page.fileLinkRightPagePath || '',
        page.isNotLinked ? 1 : 0,
        page.isDualImage ? 1 : 0,
        page.isMangaDualPage ? 1 : 0,
        page.isFileLeftDualPage ? 1 : 0,
        page.isFileRightDualPage ? 1 : 0
      );
  ...
  save(obj: LinkedFile): number {
    this.deleteByManga(obj.idManga);
    const now = new Date().toISOString();
    obj.lastAlteration = now;
    if (!obj.dateCreate) obj.dateCreate = now;
    if (!obj.lastAccess) obj.lastAccess = now;
    ...
    const result = stmt.run(
      ...
      obj.type || '',
      obj.folder || '',
      obj.language || Languages.PORTUGUESE,
      obj.dateCreate,
      obj.lastAccess,
      obj.lastAlteration
    );
    ...
    if (obj.pagesLink?.length) this.insertPages(id, obj.pagesLink);
    if (obj.pagesNotLink?.length) this.insertPages(id, obj.pagesNotLink);
    return id;
  }

  update(obj: LinkedFile): void {
    if (!obj.id) {
      this.save(obj);
      return;
    }
    ...
    if (obj.pagesLink?.length) this.insertPages(obj.id, obj.pagesLink);
    if (obj.pagesNotLink?.length) this.insertPages(obj.id, obj.pagesNotLink);
  }

  getByManga(mangaId: number): LinkedFile | undefined {
  ...
  findByFileName(mangaId: number, name: string, pages: number): LinkedFile | undefined {
    const row = this.db.prepare(`
      SELECT * FROM FileLink
  ...
  findAllByManga(mangaId: number): LinkedFile[] {
  ...
  deleteByManga(mangaId: number): void {
    this.db.prepare(`
  ...
  delete(obj: LinkedFile): void {
    if (obj.id) this.deleteById(obj.id);
  }
}
